...
const CircularSlider = ({
    ...
    (radians) => {
      ...
      if (continuous?.enabled ?? false) {
        ...
        const positiveDistance =
          (currentPoint - continuousPreviousIndex.current + clicksPerLoop) %
          clicksPerLoop;
        const negativeDistance =
          (continuousPreviousIndex.current - currentPoint + clicksPerLoop) %
          clicksPerLoop;
        ...
        const increment = positive
          ? interval * positiveDistance
          : -interval * negativeDistance;

        continuousPreviousIndex.current = currentPoint;
        const newValue = Math.min(
          max,
          Math.max(min, Number(state.label) + increment)
        );
        onChange(newValue);
        dispatch({
          type: "setKnobPosition",
  ...
  useEffect(() => {
    ...
    if (!!dataArrayLength) {
      ...
      setKnobPosition(
        -state.knobOffset + offset * getSliderRotation(direction)
      );
    }
